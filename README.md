...
/* ===== ORIGINAL VISION TEST (RESTORED FULLY) ===== */
const levels=[
...
function drawE(t,s,d){
  ...
  e.style.transform={
    up:"rotate(0deg)",right:"rotate(90deg)",
    down:"rotate(180deg)",left:"rotate(270deg)"
  }[d];
  t.appendChild(e);
}
...
/* ===== ASTIGMATISM TEST (CORRECT CLOCK DIAL) ===== */
...
function startAstigmatism(){
  ...
  app.innerHTML=`
    <div class="page active">
      <p><b>Astigmatism Clock Dial Test</b><br>Cover your left eye</p>
      <button class="button" onclick="startAstigEye('left')">Start</button>
    </div>`;
}
...
function startAstigEye(e){
  ...
  app.innerHTML=`
    <div class="page active">
      <p>Do some lines look darker or missing?</p>
      ${buildClockDial()}
...
function recordAstig(v){
  ...
  if(astigEye==="left"){
    app.innerHTML=`
      <div class="page active">
        <p>Cover your <b>right eye</b></p>
        <button class="button" onclick="startAstigEye('right')">Continue</button>
      </div>`;
  }else{
    app.innerHTML=`
      <div class="page active">
        <p><b>Astigmatism Screening Result</b><br><br>
        Left Eye: ${astigRes.left?"The screening results noticed possible signs of astigmatism. This is not a diagnosis. A comprehensive eye examination by an optometrist is recommended.":"The screening results didn't notice signs of astigmatism. This is not a diagnosis. Regular eye examinations by an optometrist are still recommended."}<br>
        Right Eye: ${astigRes.right?"The screening results noticed possible signs of astigmatism. This is not a diagnosis. A comprehensive eye examination by an optometrist is recommended.":"The screening results didn't notice signs of astigmatism. This is not a diagnosis. Regular eye examinations by an optometrist are still recommended."}</p>
      </div>`;
  }
}
...
function buildClockDial(){
  const cx = 150;
  const cy = 150;
  const innerR = 22;     // empty center circle
  const lineLen = 105;   // slightly shorter to fit numbers
  const gap = 4;

  let svg = `<svg class="astig-svg" viewBox="-10 -10 320 320">`;

  // center empty outlined circle
  svg += `
